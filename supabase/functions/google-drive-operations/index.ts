      ...
      case "upload_creative": {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (fileData) {
            ...
            // Use the creative code as the file name in Drive
            const ext = (file.file_name || file.file_path).split(".").pop() || "file";
            const driveName = files.length > 1
              ? `${creativeCode} (${i + 1}).${ext}`
              : `${creativeCode}.${ext}`;
